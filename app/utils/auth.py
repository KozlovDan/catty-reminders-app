...
from app import db_config, db_path, secret_key, storage_backend, users
...
def get_storage_for_api(username: str = Depends(get_username_for_api)):
  return _get_storage(username)


def get_storage_for_page(username: str = Depends(get_username_for_page)):
  return _get_storage(username)
